...
La tabla mide valores por defecto y no capacidades. Tres de las cuatro filas se igualan en npm escribiendo dos archivos.
...
En npm el mismo bloqueo se consigue con una línea en el archivo de configuración del proyecto. El ajuste `ignore-scripts` ya existe y viene en `false`<Cite n={5} />.
...
Un proyecto con pnpm 9 no bloquea nada. Uno con pnpm 10 avisa y sigue adelante. Uno con npm y un `.npmrc` con `ignore-scripts=true` bloquea tanto como pnpm 11. La primera comprobación es determinar cuál de esos casos aplica:
