...
Every piece of code was rebuilt for this article. Measured versions: Node.js 24.19.0, TypeScript 7.0.2, Zod 4.5.4, ESLint 10.10.0 with typescript-eslint 8.70.0, Slonik 49.10.9 and eslint-plugin-security 4.0.1.
...
One way out remains, and it has to be closed. Writing `body as CreateOrder` is a type assertion: it tells the compiler to treat the value as if it were of that type and generates no check. The rule `@typescript-eslint/no-unsafe-type-assertion` reads the same type information `tsc` does and disallows assertions that narrow a type<Cite n={10} />.
...
**How to check it.** Write the incorrect case deliberately and run `tsc` and the linter. Both have to exit 1.
