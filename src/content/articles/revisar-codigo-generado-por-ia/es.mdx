...
La medición existe. Veracode probó más de cien modelos sobre tareas de programación en cuatro lenguajes, y el 45 % de las muestras generadas falló las pruebas de seguridad. Los modelos más nuevos y más grandes no salieron mejor parados que los pequeños<Cite n={1} />. No es el defecto de un modelo concreto: es una propiedad del método.
...
Así sale. El componente esconde el botón según el rol y el endpoint borra sin preguntar nada.
...
Así sale. La consulta usa el id de la URL y nada más.
...
Así sale, sobre todo cuando el filtro llega desde un formulario de búsqueda.
...
El valor entra dentro de las comillas y puede cerrarlas. Con `x' OR '1'='1` la consulta devuelve la tabla entera, y con un punto y coma devuelve lo que el atacante quiera. La recomendación de OWASP es no llegar al intérprete: una interfaz con parámetros, o un ORM<Cite n={4} />.
...
Así sale. El asistente necesita que la variable esté disponible en el componente y le pone el prefijo que la hace pública.
...
Se comprueba sobre el resultado de la compilación, no sobre el código:
...
<Callout kind="riesgo" title="Una clave que llegó al paquete ya está quemada">
...
Así sale. El formulario limita el campo y el endpoint confía en que llegó limitado.
...
Los atributos `min`, `max` y `required` son ayudas para la persona que rellena el formulario. La petición no pasa por ellos. Con una cantidad negativa el total sale negativo y el pedido se convierte en un abono. El catálogo CWE le da número propio: CWE-602, aplicar en el cliente una protección que le corresponde al servidor<Cite n={6} />.
...
Este fallo no está en el código sino en el `package.json`. El modelo propone un paquete con un nombre verosímil que nunca se publicó.

```json
"dependencies": {
  "react-secure-input": "^2.1.0"
}
```

El tamaño del problema está medido. Un equipo de la Universidad de Texas en San Antonio generó 576.000 muestras de código con 16 modelos y encontró 205.474 nombres de paquete alucinados distintos. Al menos un 5,2 % de los paquetes citados por los modelos comerciales no existía, y un 21,7 % en los modelos abiertos<Cite n={7} />.

El nombre inventado es predecible, y quien lo registra en el registro público consigue que se instale código suyo en cualquier proyecto que siga la sugerencia. La instalación ejecuta scripts, así que el daño no espera a que alguien importe el paquete.

La comprobación va antes de instalar, no después:

```bash
npm view react-secure-input time.created maintainers repository.url
```

Hay tres respuestas malas. Un `404` significa que el paquete no existe. Una fecha de creación de hace pocos días significa que puede haberse registrado para esta campaña. Un repositorio ausente o que no corresponde al nombre significa que no hay nada que auditar.
...
Así sale, y sale así porque durante el desarrollo es cómodo.
...
El mensaje de un error de base de datos nombra la tabla, la columna y la restricción que falló. La traza nombra las rutas del servidor y las versiones de las dependencias. Es CWE-209, un mensaje de error que revela información del entorno o de sus datos<Cite n={9} />. Con eso, quien busca una vulnerabilidad ya sabe qué buscar.
