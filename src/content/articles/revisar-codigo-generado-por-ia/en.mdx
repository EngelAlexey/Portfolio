...
The invented name is predictable, and whoever registers it in the public registry gets their own code installed in every project that follows the suggestion. npm and yarn run the install scripts of dependencies, so the damage does not wait for anyone to import the package. pnpm has blocked them since version 10 and requires declaring which packages may run them.
