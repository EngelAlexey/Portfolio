...
The code an AI assistant produces compiles, passes the tests and satisfies the prompt it was given. The security model is rarely part of that prompt, so it is not part of the result either.

The failure has been measured. Veracode tested more than a hundred models on coding tasks across four languages, and 45% of the generated samples failed the security tests. Newer and larger models did no better than small ones<Cite n={1} />. The result does not depend on the particular model, but on the method used to generate the code.
...
Remembering the prefix is not a defence, because it depends on somebody recalling it every time. The defence is structural: a secret scanner in the pre-commit hook and again in continuous integration, with the keys in a secret manager instead of the repository.
...
<Callout kind="riesgo" title="A key that reached the bundle is already exposed">
...
The invented name is predictable, and whoever registers it in the public registry gets their own code installed in every project that follows the suggestion. If the package manager runs install scripts, the damage does not wait for anyone to import the package. Which ones do depends on the version: yarn 1 runs them with no exceptions, npm ran them up to version 11, and npm 12 and pnpm 10 block them unless they appear in a list declared separately. [Another article measures the four behaviours](/en/blog/npm-vs-pnpm-install-security).
...
There are three answers that argue against installing. An `E404` means the package does not exist. A creation date a few days old means it may have been registered for this campaign. A missing repository, or one that does not match the name, means there is nothing to audit.
...
The handler returns the whole error, which is convenient during development.
...
The message from a database error names the table, the column and the constraint that failed. The stack names the server paths and the dependency versions. This is CWE-209, an error message that reveals information about the environment or its data<Cite n={9} />. That information reduces the work of anyone looking for a vulnerability.
...
A log is not a private place either: it is exported, sent to a provider and read by people who do not need to see all of it. Personal data is scrubbed before it is written, not when somebody asks for access.
