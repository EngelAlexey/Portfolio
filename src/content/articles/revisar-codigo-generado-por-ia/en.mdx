...
The measurement exists. Veracode tested more than a hundred models on coding tasks across four languages, and 45% of the generated samples failed the security tests. Newer and larger models did no better than small ones<Cite n={1} />. This is not one model's defect: it is a property of the method.
...
This is how it arrives. The component hides the button by role and the endpoint deletes without asking anything.
...
This is how it arrives. The query uses the id from the URL and nothing else.
...
This is how it arrives, above all when the filter comes from a search form.
...
The value lands inside the quotes and can close them. With `x' OR '1'='1` the query returns the whole table, and with a semicolon it returns whatever the attacker asks for. The OWASP recommendation is to avoid the interpreter altogether: a parameterised interface, or an ORM<Cite n={4} />.
...
This is how it arrives. The assistant needs the variable inside the component and gives it the prefix that makes it public.
...
You check it against the build output, not against the source:
...
This is how it arrives. The form limits the field and the endpoint trusts that it arrived limited.
...
The `min`, `max` and `required` attributes help the person filling in the form. The request never passes through them. With a negative quantity the total comes out negative and the order turns into a refund. The CWE catalogue gives it a number of its own: CWE-602, client-side enforcement of a protection that belongs to the server<Cite n={6} />.
...
This failure is not in the code but in `package.json`. The model suggests a package with a plausible name that was never published.
...
The size of the problem has been measured. A team at the University of Texas at San Antonio generated 576,000 code samples with 16 models and found 205,474 distinct hallucinated package names. At least 5.2% of the packages named by commercial models did not exist, and 21.7% of those named by open-source models<Cite n={7} />.

The invented name is predictable, and whoever registers it in the public registry gets their own code installed in every project that follows the suggestion. Installation runs scripts, so the damage does not wait for anyone to import the package.
...
There are three bad answers. A `404` means the package does not exist. A creation date a few days old means it may have been registered for this campaign. A missing repository, or one that does not match the name, means there is nothing to audit.
...
This is how it arrives, and it arrives this way because it is convenient during development.
