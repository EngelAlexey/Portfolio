...
The enrolment registry ended up inside the process, so the service cannot take a second instance until that state moves to shared storage. That move is recorded as the next change.
